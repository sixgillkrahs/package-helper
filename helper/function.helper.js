...
class FunctionHelper {
  ...
  removeAllSpaceAndSpecialChars(text) {
    try {
      const newString = text.replace(/[^A-Z0-9_]+/gi, "");
      return newString;
    } catch (e) {
      return "";
    }
  ...
  getFullTextAddress(province, district, ward, address) {
    ...
    if (province && _.isNumber(province) && parseInt(province, 10) > 0) {
      const provinceObj = locationResource.LIST_PROVINCE.find(
        (x) => x.id === province
      );
      provinceText = provinceObj ? provinceObj.name : "";
    }
    if (district && _.isNumber(district) && parseInt(district, 10) > 0) {
      const districtObj = locationResource.LIST_DISTRICT.find(
        (x) => x.id === district
      );
  ...
  getFullTextAddressByLocation(locationDataObj) {
    ...
    let province =
      locationDataObj.province && locationDataObj.province.value
        ? locationDataObj.province.value.vi
        : "";
    let district =
      locationDataObj.district && locationDataObj.district.value
        ? locationDataObj.district.value.vi
        : "";
    let ward =
      locationDataObj.ward && locationDataObj.ward.value
        ? locationDataObj.ward.value.vi
        : "";
    const address =
      locationDataObj.address && locationDataObj.address.value
        ? locationDataObj.address.value.vi
        : "";
    if (province && _.isNumber(province) && parseInt(province, 10) > 0) {
      const provinceObj = locationResource.LIST_PROVINCE.find(
        (x) => x.id === province
      ...
      switch (definedProp.type) {
        case "array":
          if (!Array.isArray(paramProp) && !this.isEmpty(paramProp)) {
            ...
            res.listKeyFailed.push({
              key: `${parentName}.${key}`,
              err: `type is not ${definedProp.type}`,
            });
          }
        ...
        case "object":
          if (
            typeof paramProp !== definedProp.type &&
            !this.isEmpty(paramProp)
          ) {
            res.result = false;
            res.message = "VALIDATION FAILED";
            res.listKeyFailed.push({
  ...
  convertNumberToCurrency(
    ...
    return valueNumber.toLocaleString(locales, {
      style: "currency",
  ...
  timeBaseUUIDGenerator(length = 12) {
    ...
    if (parseInt(length) > dateString.length) {
      return (
        dateString + randomize("A0", parseInt(length) - dateString.length)
      ).toLocaleUpperCase();
    }
    return null;
  }
  ...
  formatMoneyVal(
    ...
    if (
      this.convertStringToPhone(currentUserPhone) ===
      this.convertStringToPhone(senderPhone)
    ) {
      return -value;
    } else if (
      this.convertStringToPhone(currentUserPhone) ===
      this.convertStringToPhone(receiverPhone)
    ) {
      return value;
    }
    return value;
