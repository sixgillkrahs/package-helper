...
class EncryptHelper {
	...
	desEcbEncrypt(plaintext, key) {
		key = Buffer.from(key.substr(0, 8));
		const cipher = crypto.createCipheriv("des-ecb", key, Buffer.from(0));
	...
	desEcbDecrypt(plaintext, key) {
		try {
			key = Buffer.from(key.substr(0, 8));
			const cipher = crypto.createDecipheriv("des-ecb", key, Buffer.from(0));
			cipher.setAutoPadding(true);
